...
- The dataset has two "gradient sets" — *fixed* gradient and *varied* gradient — that share the exact same geometry rows. Nearly 34% of matched fixed/varied pairs land on opposite sides of the split. For 79.4% of varied test rows the identical fixed geometry is in training; for another 10.4% it is in validation. Thus 89.8% have the fixed twin outside test, but only the first group was directly used for fitting.
...
**6. Verified a mathematical identity in the data.** There's a quantity, ⟨|∇x|⟩ (flux-surface-averaged gradient), that should be exactly reconstructible from two of the input channels. Across all 100,705 geometries the aggregate relative error is 9.4×10⁻⁸, at the representation scale expected because the registered value is stored as float32. This is valuable because it gives a **known-correct answer to test explanation methods against**: if a method claims to find what a network computes, and you feed it a case where you already know the answer, you can check whether the method lies. Three rows showed larger rowwise discrepancies and one was a clear outlier; its geometry is finite and nondegenerate, so it was noted and kept, and later steps are told to use the stored value rather than the reconstruction.
