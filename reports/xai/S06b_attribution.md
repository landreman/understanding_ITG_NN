...
- **Low-pass Integrated Gradients (IG)** as the primary path method. IG (an
  ...
  It is signed and contribution-valued. The path is a deliberately off-manifold
  diagnostic: a smoothed input need not be a realizable equilibrium.
...
fractions are only **0.7485** and **0.7452**. Thus the top 10 broadly agree about
which channels respond along this path, but they do not implement one identical
signed position-by-position mechanism. The corresponding original-function
...
The same response is larger where the members perform poorly or disagree. On
varied rows, high versus low mean-member absolute-error tertiles give 0.003789
versus 0.000759 for `gbdrift0_over_shat`; high versus low ensemble-spread
tertiles give 0.003753 versus 0.000726. On fixed rows the corresponding spread
values are 0.003434 versus 0.001018. This association is descriptive: floor
status and flux also vary across those tertiles. All stability, unstable-flux,
...
equivariance error is **9.12e−7** (range 5.05e−8–8.32e−5); canonical prediction
invariance error is median 5.49e−8. For the original network the corresponding
...
| Methods meet symmetry behavior permitted by S02 | **Pass for canonical low-pass IG; fail retained for the secondary mask.** Canonical low-pass co-shift median 9.12e−7, maximum 8.32e−5. Original median 0.874. Mask fixed-background median 0.931. See `symmetry_checks.csv`. |
| Uncertainty includes model and equilibrium sampling | **Pass.** All 168 function/method/gradient/stability/channel headline intervals use 500 joint member and whole-`equilibrium_files` resamples. |
...
| No feature is called common without an explicit agreement statistic | **Pass.** Canonical low-pass channel-rank agreement is 0.964/1.000 varied/fixed, while cell sign agreement is 0.749/0.745; the equilibrium-class contradiction is retained. |
